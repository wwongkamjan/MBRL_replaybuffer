...
class EnsembleDynamicsModel():
    ...
    def train(self, inputs, labels, weights=None, batch_size=256, holdout_ratio=0., max_epochs_since_update=5, logger=None):
        ...
        if weights is not None:
            ...
            train_weights = weights[num_holdout:]
        ...
        for epoch in itertools.count():
            ...
            for start_pos in range(0, train_inputs.shape[0], batch_size):
                if weights is not None:
                    idx = np.vstack([np.array(list(WeightedRandomSampler(train_weights, batch_size, replacement=False)))
            ...
            with torch.no_grad():
                ...
                _, self.holdout_mse_losses = self.ensemble_model.loss(holdout_mean, holdout_logvar, holdout_labels, inc_var_loss=False)
                self.holdout_mse_losses = self.holdout_mse_losses.detach().cpu().numpy()
                self.sorted_loss_idx = np.argsort(self.holdout_mse_losses)
